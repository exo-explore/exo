<script lang="ts">
	import { imageGenerationParams, setImageGenerationParams, resetImageGenerationParams, type ImageGenerationParams } from '$lib/stores/app.svelte';
	...
	const qualityOptions: ImageGenerationParams['quality'][] = ['low', 'medium', 'high'];
	const formatOptions: ImageGenerationParams['outputFormat'][] = ['png', 'jpeg', 'webp'];

	function handleSizeChange(event: Event) {
		const value = (event.target as HTMLSelectElement).value as ImageGenerationParams['size'];
		setImageGenerationParams({ size: value });
	}

	function handleQualityChange(event: Event) {
		const value = (event.target as HTMLSelectElement).value as ImageGenerationParams['quality'];
		setImageGenerationParams({ quality: value });
	}

	function handleFormatChange(event: Event) {
		const value = (event.target as HTMLSelectElement).value as ImageGenerationParams['outputFormat'];
...
<div class="border-b border-exo-medium-gray/30 px-3 py-2">
	...
	<div class="flex items-center gap-3 flex-wrap">
		...
		<div class="flex items-center gap-1.5">
			<span class="text-xs text-exo-light-gray uppercase tracking-wider">SIZE:</span>
			<select
				value={params.size}
		...
		<!-- Quality -->
		<div class="flex items-center gap-1.5">
			<span class="text-xs text-exo-light-gray uppercase tracking-wider">QUALITY:</span>
			<select
		...
		<!-- Format -->
		<div class="flex items-center gap-1.5">
			<span class="text-xs text-exo-light-gray uppercase tracking-wider">FORMAT:</span>
			<select
		...
		<button
			...
			class="flex items-center gap-1 text-xs font-mono tracking-wider uppercase transition-colors duration-200 {showAdvanced || hasAdvancedParams ? 'text-exo-yellow' : 'text-exo-light-gray hover:text-exo-yellow'}"
		>
			<span>ADVANCED</span>
			<svg
				class="w-3 h-3 transition-transform duration-200 {showAdvanced ? 'rotate-180' : ''}"
				fill="none"
				viewBox="0 0 24 24"
				stroke="currentColor"
			>
	...
	{#if showAdvanced}
		<div class="mt-3 pt-3 border-t border-exo-medium-gray/20 space-y-3">
			...
			<div class="flex items-center gap-4 flex-wrap">
				<!-- Seed -->
				<div class="flex items-center gap-1.5">
					<span class="text-xs text-exo-light-gray uppercase tracking-wider">SEED:</span>
					<input
						type="number"
						min="0"
						value={params.seed ?? ''}
				...
				<div class="flex items-center gap-1.5 flex-1 min-w-[200px]">
					<span class="text-xs text-exo-light-gray uppercase tracking-wider whitespace-nowrap">STEPS:</span>
			...
			<!-- Row 2: Guidance -->
			<div class="flex items-center gap-1.5">
				<span class="text-xs text-exo-light-gray uppercase tracking-wider whitespace-nowrap">GUIDANCE:</span>
			...
			<div class="flex flex-col gap-1.5">
				<span class="text-xs text-exo-light-gray uppercase tracking-wider">NEGATIVE PROMPT:</span>
				<textarea
