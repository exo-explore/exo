...
async def test_auto_dial_empty_env():
    ...
    with patch.dict(os.environ, {}, clear=False):
        ...
        await router._auto_dial_peers()
    router._net.dial.assert_not_called()
    assert router._persistent_peers == []
...
async def test_auto_dial_parses_peers():
    ...
    with patch.dict(os.environ, {"EXO_PEERS": addrs}):
        ...
        with patch("exo.routing.router.sleep", new_callable=AsyncMock):
            await router._auto_dial_peers()
    assert len(router._persistent_peers) == 2
    assert router._persistent_peers[0] == "/ip4/192.168.2.2/tcp/51821/p2p/QmFoo"
    assert router._persistent_peers[1] == "/ip4/10.0.0.152/tcp/51821/p2p/QmBar"
...
async def test_auto_dial_skips_when_already_connected():
    ...
    router = _make_router()
    router._connected_peer_ids.add("QmAlreadyHere")
...
def test_peer_tracking_deduplicates_connect():
    ...
    router = _make_router()
    router._connected_peer_ids.add("QmPeer1")
    assert len(router._connected_peer_ids) == 1
    router._connected_peer_ids.add("QmPeer1")  # duplicate
    assert len(router._connected_peer_ids) == 1
...
def test_peer_tracking_discard_on_disconnect():
    ...
    router = _make_router()
    router._connected_peer_ids.discard("QmNeverSeen")  # must not raise
    assert len(router._connected_peer_ids) == 0
