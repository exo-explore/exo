...
for text generation while keeping image generation MLX-specific (lazy loaded).
...
Tasks:
  ...
  - ImageGeneration: Generate images (MLX-only, lazy imports)
  - ImageEdits: Edit images (MLX-only, lazy imports)
...
if TYPE_CHECKING:
    from exo.worker.engines.image import DistributedImageModel
...
def main(
    ...
    """
    Main runner loop using Engine abstraction for text generation.

    Args:
        bound_instance: The bound instance configuration.
        event_sender: Channel to send events to the worker.
        task_receiver: Channel to receive tasks from the worker.
        engine: The inference engine (MlxEngine, PytorchEngine, etc.)
    """
    ...
    with task_receiver as tasks:
        for task in tasks:
            ...
            match task:
                ...
                case LoadModel() if (
                    ...
                    if ModelTask.TextGeneration in shard_metadata.model_card.tasks:
                        ...
                        # Image generation is MLX-only - lazy import
                ...
                case StartWarmup() if isinstance(current_status, RunnerLoaded):
                    ...
                    if ModelTask.TextGeneration in shard_metadata.model_card.tasks:
                        ...
                        # Image warmup is MLX-only - lazy import
                ...
                case TextGeneration(task_params=task_params, command_id=command_id) if (
                    ...
                    try:
                        ...
                        # Use engine for text generation - all patches applied internally
                        completion_tokens = 0
                ...
                case ImageEdits(task_params=task_params, command_id=command_id) if (
                    ...
                    # Image edits is MLX-only - lazy import
                    from exo.worker.engines.image import generate_image
            ...
            if isinstance(current_status, RunnerShutdown):
                # Use engine cleanup
                engine.cleanup()
                # Also cleanup image model if used
...
# =============================================================================
# Helper functions for image generation (MLX-only, kept here for now)
# =============================================================================
