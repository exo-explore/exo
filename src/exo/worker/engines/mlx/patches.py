...
NOTE: These patches are DUPLICATED in runner.py (upstream's implementation).
When runner.py is refactored to use the Engine abstraction, these patches
will become the authoritative source. Until then, runner.py has its own copies.
"""
...
def patch_kimi_tokenizer(tokenizer: TokenizerWrapper) -> None:
    ...
    Updated to match upstream's implementation with tool ID support.
    """
