...
class JointBlockWrapper(ABC):
    ...
    def __init__(self, block: Any, text_seq_len: int):
        ...
        self._kv_cache: ImagePatchKVCache | None = None
    ...
    def _cache_full_image_kv(self, img_key: mx.array, img_value: mx.array) -> None:
        ...
        self._kv_cache.update_image_patch(0, img_key.shape[2], img_key, img_value)

    def _cache_patch_kv(self, img_key: mx.array, img_value: mx.array) -> None:
        """Store current patch's K/V during PATCHED mode."""
        assert self._kv_cache is not None
        self._kv_cache.update_image_patch(
            self._patch_start, self._patch_end, img_key, img_value
        )
    ...
    def _get_full_kv(
        ...
        assert self._kv_cache is not None
        return self._kv_cache.get_full_kv(text_key, text_value)
