...
from typing import List, Callable, Protocol, cast

# Mock ShardMetadata (will be replaced by the actual one from exo.shared.types.worker.shards)
class PipelineShardMetadata:
    def __init__(self, start_layer: int, end_layer: int, device_rank: int, world_size: int):
...
def _get_layers(model: nn.Module) -> tuple[nn.ModuleList, list[str]]:
    ...
    """
    for attr_path, description in SUPPORTED_LAYER_PATTERNS:
...
def _set_layers(model: nn.Module, new_layers: nn.ModuleList, attr_path: list[str]) -> None:
...
def pipeline_auto_parallel(
    ...
    if sliced_layers: # Ensure there are layers to wrap
        sliced_layers[0] = PipelineFirstLayer(sliced_layers[0], device_rank, group=group)
        sliced_layers[-1] = PipelineLastLayer(sliced_layers[-1], device_rank, world_size, group=group)
