    ...
    if shutil.which("nvidia-smi"):
        try:
            ...
            result = await run_process(
                ["nvidia-smi", "--query-gpu=utilization.gpu,power.draw,temperature.gpu,memory.total,memory.free", "--format=csv,noheader,nounits"]
            )
...
async def get_linux_metrics_async() -> MacmonMetrics:
    ...
    gpu_util_decimal = gpu_metrics.gpu_utilization / 100.0 if gpu_metrics.gpu_utilization > 0 else 0.0
