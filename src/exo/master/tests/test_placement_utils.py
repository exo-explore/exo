...
def test_get_shard_assignments_bandwidth_aware():
    ...
    # act
    shard_assignments = get_shard_assignments(
        model_meta, selected_cycle, Sharding.Pipeline, node_memory, node_bandwidth
    )
...
def test_get_shard_assignments_falls_back_without_bandwidth():
    ...
    # Create nodes with different RAM amounts
    ...
    # act - no bandwidth data provided
    shard_assignments = get_shard_assignments(
        model_meta, selected_cycle, Sharding.Pipeline, node_memory
    )

    # assert - should use RAM-proportional assignment
    runner_id_a = shard_assignments.node_to_runner[node_a_id]
    runner_id_b = shard_assignments.node_to_runner[node_b_id]
    ...
    # With 500KB vs 1000KB (1:2 ratio), expect ~4 and ~8 layers
    assert layers_a == 4
    assert layers_b == 8
