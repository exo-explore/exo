...
def test_custom_model_chat():
    ...
    api_url = os.environ.get(
        "EXO_API_URL", "http://localhost:52415/v1/chat/completions"
    )
...
def run_custom_model_test(model: str, prompt: str, api_url: str):
    ...
    def check_for_instance(model_id: str) -> bool:
        try:
            ...
            instances = cast(
                Dict[str, Dict[str, Dict[str, Any]]], state_data.get("instances", {})
            )

            for inst_wrapper in list(instances.values()):
                for actual_inst in list(inst_wrapper.values()):
                    shard_assignments = cast(
                        Dict[str, Any], actual_inst.get("shardAssignments", {})
                    )
                    if shard_assignments.get("modelId") == model_id:
                        return True
        except requests.exceptions.ConnectionError:
            print(
                "Error: Could not connect to EXO. Make sure 'uv run exo' is running in the background."
            )
