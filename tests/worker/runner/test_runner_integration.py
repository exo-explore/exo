...
import asyncio
import sys
from exo.shared.types.worker.instances import Instance, MlxRingInstance, ShardAssignments
...
from exo.shared.types.tasks import LoadModel, TaskId, ChatCompletion, ChatCompletionTaskParams, StartWarmup
from exo.shared.types.api import ChatCompletionMessage
from exo.shared.types.events import ChunkGenerated

@pytest.mark.asyncio
@pytest.mark.skipif(sys.platform != "darwin", reason="MLX only available on macOS/Apple Silicon")
async def test_runner_supervisor_with_mlx_engine():
    ...
    async with anyio.create_task_group() as tg:
            ...
            async with event_receiver:
                async for event in event_receiver:
                    ...
                    if isinstance(event, ChunkGenerated):
                        print(f"Received chunk: {event.chunk.text}")
        ...
        await supervisor.start_task(chat_task)
