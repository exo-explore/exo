...
class TestGetLinuxGpuMetrics:
    ...
    async def test_nvidia_smi_not_found(self):
        ...
        with patch("exo.utils.info_gatherer.linux_metrics.shutil.which", return_value=None):
    ...
    async def test_nvidia_smi_normal_output(self):
        ...
        with patch("exo.utils.info_gatherer.linux_metrics.shutil.which", return_value="/usr/bin/nvidia-smi"):
            with patch("exo.utils.info_gatherer.linux_metrics.run_process", new_callable=AsyncMock, return_value=mock_result):
                metrics = await get_linux_gpu_metrics()
    ...
    async def test_nvidia_smi_exception(self):
        ...
        with patch("exo.utils.info_gatherer.linux_metrics.shutil.which", return_value="/usr/bin/nvidia-smi"):
            with patch("exo.utils.info_gatherer.linux_metrics.run_process", new_callable=AsyncMock, side_effect=Exception("Driver error")):
                metrics = await get_linux_gpu_metrics()
                assert metrics.gpu_utilization == 0.0
                assert metrics.vram_total_bytes == 0
