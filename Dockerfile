...
# 1. Create venv
# 2. Sync dependencies from lockfile
# 3. Install the pre-built native extension wheel
# 4. Install the project itself without deps (they're already installed)
# 5. Install MLX v0.30.4 with CUDA quantized model support (PR #2984)
RUN uv venv --python python3.13 \
    ...
    # Install MLX with CUDA quantized model support (PR #2984)
...
#     CMD curl -f http://localhost:52415/health || exit 1
