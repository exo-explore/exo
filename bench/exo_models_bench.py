    ...
    def _make_counter(tokenizer: Any) -> Callable[[str], int]:
        def count_fn(user_content: str) -> int:
            ...
            try:
                ...
                from vendor.deepseek_v4_encoding import (
...
def _run_model(client: ExoClient, args: argparse.Namespace) -> dict[str, Any]:
    """Run the full benchmark for args.model; return the output dict."""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="exo-models-bench",
        description="Benchmark exo model throughput for multiple models from a JSON file.",
    )
    add_common_instance_args(ap)
    # --model is required by add_common_instance_args, but --models-file can replace it
    for _action in ap._actions:
        if "--model" in getattr(_action, "option_strings", []):
            _action.required = False
            _action.default = None
            break
    ap.add_argument(
        "--models-file",
        default=None,
        help="JSON file with a 'models' list. Runs benchmark for each model in order.",
    )
    ap.add_argument(
        "--pp",
        nargs="+",
    ...
    ap.add_argument(
        "--excel-out",
        default=None,
        help="Write all models' results to a single Excel file (default: results_all_{datetime}.xlsx).",
    )
    ...
    # Validate early before hitting the network
    pp_list = parse_int_list(args.pp)
    tg_list = parse_int_list(args.tg)
    ...
    if args.models_file:
        with open(args.models_file) as f:
            models_to_run: list[str] = json.load(f)["models"]
        if args.json_out:
            logger.warning("--json-out ignored when --models-file is used; results auto-named per model.")
    elif args.model:
        models_to_run = [args.model]
    else:
        logger.error("Either --model or --models-file is required.")
        return 2
    ...
    for model_name in models_to_run:
        ...
        if args.stdout:
            ...
            json_out = (
                (args.json_out if not args.models_file else None)
                or f"results_{model_name.replace('/', '_')}_{time.strftime('%Y%m%d_%H%M%S')}.json"
            )
            with open(json_out, "w", encoding="utf-8") as f:
                json.dump(output, f, indent=2, ensure_ascii=False)
            logger.info(f"Wrote results JSON: {json_out}")
        all_excel_rows.extend(output.get("runs", []))

    if all_excel_rows:
        excel_out = args.excel_out or f"results_all_{time.strftime('%Y%m%d_%H%M%S')}.xlsx"
        pd.json_normalize(all_excel_rows).to_excel(excel_out, index=False, sheet_name="results")
