...
async def profile_inference(
    ...
    _ = await engine.generate(shard, encoded_prompt, max_tokens=20)
...
async def main():
    ...
    # Print results table
    print("\n=== Results ===")
    print(f"{'Model':<15} {'Quant':<8} {'Avg Latency':<12} {'Tokens/sec':<10}")
    print("-" * 50)
    for r in results:
        print(f"{r['model']:<15} {r['quantization']:<8} {r['avg_latency']:.2f}s {r['tokens_per_second']:.2f}")
