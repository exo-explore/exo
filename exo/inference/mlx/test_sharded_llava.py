...
class LlavaShardTest(unittest.TestCase):
    def setUp(self):
        ...
        self.shard2 = Shard(model_id=model_id, start_layer=13, end_layer=31,
                            n_layers=32)
        self.prompts = ["USER: <image>\nWhat are these?\nASSISTANT:",
                        "USER: <image>\nHow many dogs are they?\nASSISTANT:"]
        self.imgs = ["http://images.cocodataset.org/val2017/000000039769.jpg",
                     "http://images.cocodataset.org/val2017/000000039769.jpg"]
        self.max_tokens = 30

    async def _test_full_shard(self):
        resp, inference_state, _ = await self.inference_engine.infer_prompt(["A", "B"], shard=self.full_shard,
                                                                            prompts=self.prompts, image_strs=self.imgs)
