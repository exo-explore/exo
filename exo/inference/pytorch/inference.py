...
#         return (
#             output_data,
#             json.dumps({"start_pos": start_pos, "past_key_values": past_key_values}),
#             output_data.size == 1 and output_data.item() in [self.tokenizer.eos_token_id],
#         )
...
class PyTorchDynamicShardInferenceEngine(InferenceEngine):
    ...
    async def infer_prompt(
        ...
        output_data, past_key_values = self.model.forward_layers(last_tok, past_key_values=[])
        output_data = output_data.detach().cpu().numpy()
    ...
    async def infer_tensor(
        ...
        output_data, past_key_values = self.model.forward_layers(torch.tensor([input_data], device=self.model.device), past_key_values=[])
        output_data = output_data.detach().cpu().numpy()
