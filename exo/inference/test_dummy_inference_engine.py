...
async def test_dummy_inference_engine_random():
    ...
    dummy_engine = DummyInferenceEngine(output_type="random", output_shape=(2, 2), latency_mean=0.1, latency_stddev=0.01)
    await dummy_engine.run_inference()  # Simulate inference, check for errors
    output = dummy_engine.output_value
    assert isinstance(output, list), "Output should be a list."
    assert len(output) == 2, "Output should have the specified shape."
