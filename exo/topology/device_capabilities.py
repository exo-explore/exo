...
class DeviceCapabilities:
    ...
    def __str__(self) -> str:
        return (
            ...
            f"Available Memory: {self.memory_available}MB. "
            f"FLOPS: {self.flops}"
        )
...
def device_capabilities() -> DeviceCapabilities:
    if psutil.MACOS:
        ...
        return DeviceCapabilities(
            ...
            memory_available=psutil.virtual_memory().available // (2**20),
            flops=DeviceFlops(fp32=0.0, fp16=0.0, int8=0.0),
        )
...
def mac_device_capabilities() -> DeviceCapabilities:
    ...
    model_id = model_line.split(": ", 1)[1].strip() if model_line else "Unknown Model"

    chip_line = next((line for line in output.split("\n") if "Chip" in line), None)
    chip_id = chip_line.split(": ", 1)[1].strip() if chip_line else "Unknown Chip"

    memory_total = psutil.virtual_memory().total // (2**20)  # Total system memory in MB
    memory_available = psutil.virtual_memory().available // (2**20)  # Available system memory in MB

    flops = CHIP_FLOPS.get(chip_id, DeviceFlops(fp32=0.0, fp16=0.0, int8=0.0))
...
def linux_device_capabilities() -> DeviceCapabilities:
    ...
    gpu_device_types = {"CUDA", "NV", "GPU"}

    if Device.DEFAULT in gpu_device_types:
        init_succeeded = False
        try:
            pynvml.nvmlInit()
            init_succeeded = True
            handle = pynvml.nvmlDeviceGetHandleByIndex(0)
            gpu_name = pynvml.nvmlDeviceGetName(handle).decode().strip().upper()
            memory_info = pynvml.nvmlDeviceGetMemoryInfo(handle)
            gpu_memory_total = memory_info.total // (2**20)        # Total GPU memory in MB
            gpu_memory_available = memory_info.free // (2**20)     # Available GPU memory in MB

            if DEBUG >= 2:
                print(
                    f"NVIDIA device gpu_name={gpu_name} "
                    f"gpu_memory_total={gpu_memory_total}MB "
                    f"gpu_memory_available={gpu_memory_available}MB"
                )

            flops = CHIP_FLOPS.get(gpu_name, DeviceFlops(fp32=0.0, fp16=0.0, int8=0.0))

            return DeviceCapabilities(
                model=f"Linux Box ({gpu_name})",
                chip=gpu_name,
                memory=gpu_memory_total,
                memory_available=gpu_memory_available,
                flops=flops,
            )
        except Exception as error:
            if DEBUG >= 1:
                print(f"Failed to retrieve NVIDIA device information: {error}")
        finally:
            if init_succeeded:
                try:
                    pynvml.nvmlShutdown()
                    ...
                    if DEBUG >= 1:
                        print("Failed to shutdown NVML properly.")

    # Handle non-GPU devices
    memory_total = psutil.virtual_memory().total // (2**20)          # Total system memory in MB
    memory_available = psutil.virtual_memory().available // (2**20)  # Available system memory in MB
    flops = DeviceFlops(fp32=0.0, fp16=0.0, int8=0.0)
